(async function () {
  ...
  let clientsList = await getClientsData();
  let prop = "id";
  let dir = false;

  const tableClients = createTable();
  const tableBody = tableClients.tableBody;
  const containerLoader = tableClients.tableContainerLoader;
  const loader = tableClients.containerLoader;
  const clientNotFound = tableClients.tableClientNotFound;
  containerLoader.classList.add("container-loader--visible");
  clientNotFound.classList.add("client-not-found--visible");
  ...
  async function responseValid(response, clientData) {
    ...
    if (clientForm.formCaption.textContent !== "Удалить клиента") {
      if (
        clientData.contacts.some(
          (contact) => contact.type === "Email" && !contact.value.includes("@")
        )
      ) {
        let error = createElem("div");
        error.textContent = `Ошибка: корректно заполните email`;
  ...
  function createTable() {
    ...
    const containerLoader = createElem("div");
    const tableClientNotFound = createElem("div");
    ...
    tableClientNotFound.classList.add("client-not-found--visible");
    tableClientNotFound.textContent = "Клиент не найден!";
    ...
    tableContainerLoader.append(containerLoader, tableClientNotFound);
  ...
  function createTableRow(clientItem) {
    ...
    clientItem.contacts.forEach((item) => {
      ...
      if (item.type.includes("Телефон") || item.type.includes("Доп")) {
    ...
    clientId.textContent = clientItem.id.slice(0, 9);
  ...
  function tableAdd(clientsList, clientForm, tableContainer) {
    ...
    containerLoader.classList.remove("container-loader--visible");
  ...
  function tableButtonAction(
    ...
    buttonMain.addEventListener("click", function () {
      ...
      clientForm.buttonCancelAdd.style.display = "block";
      clientForm.buttonClientDelete.style.display = "none";
    ...
    buttonChange.addEventListener("click", function () {
      ...
      clientForm.formContainerError.innerHTML = "";
      clientForm.buttonClientAdd.textContent = "Сохранить";
      clientForm.buttonCancelAdd.style.display = "none";
      clientForm.buttonClientDelete.style.display = "block";
  ...
  function createForm() {
    ...
    const buttonClientDelete = createElem("div");
    ...
    buttonClientDelete.classList.add("btn-reset", "form-btn--delete");

    formCaption.textContent = "Добавить клиента";

    formContainerTextDelete.textContent =
      "Вы действительно хотите удалить клиента?";
    formContainerTextDelete.style.display = "none";
    ...
    buttonClientAdd.textContent = "Сохранить";
    buttonCancelAdd.textContent = "Отмена";
    buttonClientDelete.textContent = "Удалить клиента";
  ...
  function formVisible() {
    ...
    clientForm.formHeaderBtnClose.addEventListener("click", () => {
      modalOverlay.classList.remove("modal-overlay--visible");
      modal.classList.remove("modal--visible");
    });

    clientForm.buttonCancelAdd.addEventListener("click", () => {
      modalOverlay.classList.remove("modal-overlay--visible");
      modal.classList.remove("modal--visible");
    });
  ...
  async function formAction(clientForm) {
    ...
    clientForm.buttonClientDelete.addEventListener("click", function () {
      modal.classList.add("form-delete");
      clientForm.formHeader.classList.add("form-delete-header");
      clientForm.formCaption.textContent = "Удалить клиента";
      clientForm.buttonClientAdd.textContent = "Удалить";
  ...
  async function clientSearch(tableContainer, clientForm) {
    ...
    searchInput.addEventListener("input", function () {
      ...
      clientNotFound.classList.add("client-not-found--visible");
      clientNotFound.classList.remove("client-not-found");

      timerFilter = setTimeout(async function () {
        for (let client of clientsListFilter) {
          client.searchprop =
            client.surname.toLowerCase() +
            " " +
        ...
        if (clientsListFilter.length === 0) {
          ...
          setTimeout(() => {
            ...
            clientNotFound.classList.remove("client-not-found--visible");
            clientNotFound.classList.add("client-not-found");
          }, 300);
        } else {
          loader.classList.remove("loader--visible");
          clientNotFound.classList.add("client-not-found--visible");
